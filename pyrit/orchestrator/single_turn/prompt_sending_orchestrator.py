...
class PromptSendingOrchestrator(Orchestrator):
    ...
    async def output_conversation_to_file(self, file_path: str = "conversation.txt"):
        """Outputs the conversation to a file."""
        messages = self.get_memory()
        last_conversation_id = None
        ...
        with open(file_path, "w") as f:
            ...
            for message in messages:
                ...
                if message.role == "user" or message.role == "system":
                    f.write(f"{message.role}: {message.converted_value}\n")
                else:
                    f.write(f"{message.role}: {message.converted_value}\n")
